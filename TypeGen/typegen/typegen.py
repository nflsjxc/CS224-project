...
import traceback
import argparse
from config import openai_keys, max_retry, error_sleep, normal_sleep, datafiles, chatgpt, gpt3, gpt4
import random

api_index = 0
client = None
# api_key = "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"
api_key = "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"
...
def run_gpt3(prompt_str):
    return client.completions.create(model=gpt3,
    prompt=prompt_str,
    temperature=1,
    max_tokens=128,
    top_p=1.0,
...
def run(data_folder, resfile, model, demo_num = 5, usertype = True, hop = 3, api_index = api_index, cont = False, fix_demo = False, cot = True, slice_code = True):
    ...
    with open('input_100_seed2.json', 'w') as f:
        json.dump(testset, f)
            ...
            while Flag_no_error and num < max_retry:
                try:
                    ...
                    if 'You exceeded your current quota' in str(e):
                        ...
                        api_index += 1
                        print("error at api_key")
                        # openai.api_key = keys[api_index]
                        # api_key = keys[api_index]
                        # num -= 1
