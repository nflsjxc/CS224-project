...
class FTEmbedding:
    """
    FastText embeddings for code tokens and identifiers
    """
    def __init__(self, param_df: pd.DataFrame, return_df: pd.DataFrame,
                 var_df: pd.DataFrame, ft_model_tk_path) -> None:
        self.param_df = param_df
        self.return_df = return_df
        self.var_df = var_df
        self.ft_model_tk_path = ft_model_tk_path

    def train_model(self, corpus_iterator: TokenIterator, model_path_name: str) -> None:
        """
        Train a FastText model and save the output to a file.
        ...
        ft_model = FastText(min_count=5,
                             window=5,
                             vector_size=FT_VEC_LENGTH,
                             workers=multiprocessing.cpu_count())

        t = time()
        ft_model.build_vocab(corpus_iterable=corpus_iterator)
        logger.info('Built FT vocab in {} mins'.format(round((time() - t) / 60, 2)))
        logger.info(f"FT model's vocab size: {len(ft_model.wv):,}")

        t = time()
        ft_model.train(corpus_iterable=corpus_iterator,
                        total_examples=ft_model.corpus_count,
                        epochs=20,
                        report_delay=1)

        logger.info('Built W2V model in {} mins'.format(round((time() - t) / 60, 2)))
        ft_model.save(model_path_name)

    def train_token_model(self):
        """
        Trains a FastText model for tokens.
        """

        self.train_model(TokenIterator(self.param_df, self.return_df, self.var_df), self.ft_model_tk_path)
...
def vectorize_sequence(sequence: str, feature_length: int, w2v_model: FastText) -> np.ndarray:
    """
    Vectorize a sequence to a multi-dimensial NumPy array
    """
...
def vectorize_args_ret(output_path: str):
    ...
    if not os.path.exists(os.path.join(output_path, 'ft_token_model.bin')):
        embedder = FTEmbedding(train_param_df, train_return_df, train_var_df,
                                os.path.join(output_path, 'ft_token_model.bin'))
        embedder.train_token_model()
    else:
        logger.warn("Loading an existing pre-trained W2V model!")

    #w2v_token_model = Word2Vec.load(os.path.join(output_path, 'w2v_token_model.bin'))
    w2v_token_model = FastText.load(os.path.join(output_path, 'ft_token_model.bin'))
