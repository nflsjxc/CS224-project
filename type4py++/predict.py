...
def test(output_path: str, data_loading_funcs: dict, type_vocab_limit: int=None, use_tc_reduced: bool=False,
         model_encoder_name:str="LSTM",read_cluster_result:bool=False):
    ...
    if read_cluster_result:
        logger.info("Loading the type clusters")
        annoy_index = AnnoyIndex(model.model.output_size, 'euclidean')
        embed_labels = np.load(join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_true.npy"))
        annoy_index.load(join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_type_cluster"))
    else:
        if not use_tc_reduced:
            ...
            annoy_index.save(join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_type_cluster"))
            np.save(join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_true.npy"), embed_labels)
    ...
    save_json(join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_test_predictions.json"), pred_types)
