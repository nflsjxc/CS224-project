...
def evaluate(output_path: str, data_name: str, tasks: set ,top_n: int=10, mrr_all=False,model_encoder_name:str="LSTM"):

    logger.info(f"Evaluating the Type4Py {data_name} model for {tasks} prediction task")
    logger.info(f"*************************************************************************")
    # Loading label encoder andd common types
    test_pred = load_json(join(output_path, f'type4py_{data_name}_{model_encoder_name}_test_predictions.json'))
