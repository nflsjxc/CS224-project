...
class Type4Py_GRU(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, aval_type_size: int,
        ...
        # 使用 GRU 替换 LSTM
        self.gru_id = nn.GRU(self.input_size, self.hidden_size, self.num_layers, batch_first=True,
                             bidirectional=True)
        self.gru_tok = nn.GRU(self.input_size, self.hidden_size, self.num_layers, batch_first=True,
                              bidirectional=True)
    ...
    def forward(self, x_id, x_tok, x_type):
        ...
        x_id = self.dropout(x_id) # [bs,seq_len,feature_dim:100]
        x_tok = self.dropout(x_tok) # [bs,seq_len,feature_dim:100]
        
        # GRU forward pass
        x_id, _ = self.gru_id(x_id) # [(batch_size, seq_len, num_directions * hidden_size)]
        x_tok, _ = self.gru_tok(x_tok)
...
class Type4Py_GRU_maxPoolint(nn.Module):
    """
    Complete model
    """
    def __init__(self, input_size: int, hidden_size: int, aval_type_size: int,
    ...
    def forward(self, x_id, x_tok, x_type):
        ...
        # Decode the hidden state of the last time step
        x_id,_ = torch.max(x_id,dim=1)
        x_tok,_ = x_tok.max(dim=1)
...
class Type4Py_transformer(nn.Module):
    """
    Complete model with Transformer
    """
    def __init__(self, input_size: int, hidden_size: int, aval_type_size: int,
                 num_layers: int, output_size: int, dropout_rate: float):
        super(Type4Py_transformer, self).__init__()
        ...
        # Transformer Layers
        self.input_linear_id = nn.Linear(self.input_size, self.hidden_size * 2)
        self.input_linear_tok = nn.Linear(self.input_size, self.hidden_size * 2)
        
        self.transformer_id = nn.TransformerEncoderLayer(d_model=self.hidden_size * 2, nhead=self.hidden_size * 2 // 8, dim_feedforward=hidden_size, dropout=dropout_rate)
        self.transformer_tok = nn.TransformerEncoderLayer(d_model=self.hidden_size * 2, nhead=self.hidden_size * 2 // 8, dim_feedforward=hidden_size, dropout=dropout_rate)

        self.encoder_id = nn.TransformerEncoder(self.transformer_id, num_layers=num_layers)
        self.encoder_tok = nn.TransformerEncoder(self.transformer_tok, num_layers=num_layers)
    ...
    def forward(self, x_id, x_tok, x_type):
        ...
        x_id = self.input_linear_id(x_id)
        x_tok = self.input_linear_id(x_tok)
        
        # Transformer Encodings
        x_id = self.encoder_id(x_id.permute(1,0,2))
        x_tok = self.encoder_tok(x_tok.permute(1,0,2))
    
        
        x_id = x_id.permute(1,0,2)
        x_tok = x_tok.permute(1,0,2)
        
        
        # Decode the hidden state of the last time step
        x_id = x_id[:, -1, :] # # [bs,feature_dim:100]
        x_tok = x_tok[:, -1, :]
...
class Type4Py_transformer_attentionFusion(nn.Module):
    """
    Complete model with Transformer
    """
    def __init__(self, input_size: int, hidden_size: int, aval_type_size: int,
                 num_layers: int, output_size: int, dropout_rate: float):
        super(Type4Py_transformer_attentionFusion, self).__init__()
...
class Type4Py_fusion(nn.Module):
    """
    Complete model
    """
    def __init__(self, input_size: int, hidden_size: int, aval_type_size: int,
                 num_layers: int, output_size: int, dropout_rate: float):
        super(Type4Py_fusion, self).__init__()
    ...
    def forward(self, x_id, x_tok, x_type):
        ...
        x_id, _ = self.lstm_id(x_id) # [(batch_size, seq_len, num_directions * hidden_size)]
...
def load_model(model_type: str, model_params: dict,model):
    """
    ...
    if model_type == "woi":
        ...
        if model == "LSTM":
            ...
            return Type4Py_transformer(W2V_VEC_LENGTH, model_params['hidden_size'], AVAILABLE_TYPES_NUMBER, model_params['layers'],
                               model_params['output_size'], model_params['dr']).to(DEVICE)
        elif model == "GRU":
            return Type4Py_GRU(W2V_VEC_LENGTH, model_params['hidden_size'], AVAILABLE_TYPES_NUMBER, model_params['layers'],
                               model_params['output_size'], model_params['dr']).to(DEVICE)
        elif model == "transformer_attentionFusion":
            return Type4Py_transformer_attentionFusion(W2V_VEC_LENGTH, model_params['hidden_size'], AVAILABLE_TYPES_NUMBER, model_params['layers'],
                               model_params['output_size'], model_params['dr']).to(DEVICE)
        elif model == "LSTM_max_pooling":
            return Type4Py_fusion(W2V_VEC_LENGTH, model_params['hidden_size'], AVAILABLE_TYPES_NUMBER, model_params['layers'],
                               model_params['output_size'], model_params['dr']).to(DEVICE)
        elif model == "GRU_max_pooling":
            return Type4Py_GRU_maxPoolint((W2V_VEC_LENGTH, model_params['hidden_size'], AVAILABLE_TYPES_NUMBER, model_params['layers'],
                               model_params['output_size'], model_params['dr']).to(DEVICE))
        ...
        if valid_data_loader is not None:
            if epoch % 5 == 0:
                ...
                if valid_loss < best_loss:
                    ...
                    logger.info("Saved the trained Type4Py model for prediction on the disk" )
                    torch.save(model.module if torch.cuda.device_count() > 1 else model, model_path)
...
def train(output_path: str, data_loading_funcs: dict, model_params_path=None, validation:bool=False,
          model_encoder_name:str="LSTM",is_rc:bool=False):
    ...
    if is_rc:
        try:
            logger.info("read checkpoint form disk")
            model = torch.load(join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_model.pt"))
        except:
            logger.info("there is not checkpoint")
            exit()
    else:
        model = load_model(data_loading_funcs['name'], model_params,model_encoder_name)
    ...
    train_loop_dsl(model, criterion, optimizer, train_data_loader,
                   ...
                   model_params['epochs'], ubiquitous_types, common_types, join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_model.pt"))
    ...
    if not validation:
        ...
        torch.save(model.module if torch.cuda.device_count() > 1 else model, join(output_path, f"type4py_{data_loading_funcs['name']}_{model_encoder_name}_model.pt"))
